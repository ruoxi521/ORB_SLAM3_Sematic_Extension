...
class Receiver
{
    ...
    private:
        ...
        float mcx=0,mcy,mfx=0,mfy=0;

        pcl::VoxelGrid<PointT> voxel;       // 点云显示精度
        pcl::PassThrough<PointT> passthrough;

        size_t lastKeyframeSize = 0;        // 临时变量
        size_t mGlobalPointCloudID = 0;     // 点云ID
        size_t mLastGlobalPointCloudID = 0; // 用于指示建图处理到某一位置
        ...
        typedef message_filters::sync_policies::ExactTime<sensor_msgs::PointCloud2, 
                                                          geometry_msgs::PoseStamped> ExactSyncPolicy_2;
        typedef message_filters::sync_policies::ApproximateTime<sensor_msgs::PointCloud2, 
                                                          geometry_msgs::PoseStamped> ApproximateSyncPolicy_2;
...
pcl::PointCloud<Receiver::PointT>:: Ptr Receiver::generatePointCloud(int index_i)
{
...
void Receiver::callback_pointcloud(const sensor_msgs::PointCloud2::ConstPtr msgPointCloud,
                         const geometry_msgs::PoseStamped::ConstPtr tcw)
{
...
{
    if( Receiver::globalMap->points.size()>0)
    {	
        // cout<<"gogogo"<<endl;
        string save_path = "../../../pcd/resultPointCloudFile_KITTI.pcd";
